...
This topic describes how to enable server object extensions and server object interceptors on map services (published using ArcGIS Pro) in ArcGIS Server Manager and Portal Home Application.
...
If the SOE or SOI is deployed before the map service is published, you can enable it via ArcGIS Pro. Otherwise, you need to enable the SOE or SOI through the ArcGIS Server Manager or the Portal Home Application.
