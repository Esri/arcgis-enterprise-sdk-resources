...
### 2. Migrate your extensions to be built with Enterprise SDK
  - For .NET SOEs and SOIs, see [Migrate older .NET SOE or SOI projects to 11](../../net/net-8-support/#migrate-older-net-soe-or-soi-projects-to-11).
  - For Java SOEs and SOIs, see [Migrate extensions (from Java project to Maven project)](../upgrade-extensions/#migrate-extensions-from-java-project-to-maven-project).
...
### 3. Deploy the migrated extension to ArcGIS Server
  ...
  After the extension is successfully migrated and built with Enterprise SDK in step 2, you can deploy it to ArcGIS Server. See [Deploy extensions](../deploy-extensions-java/) for detailed instructions.

### 4. Enable the extension with Pro-based services

  Find the services that are migrated successfully in step 1, and then enable the migrated extension with it. See [Enable extensions](../enable-extensions-java/) for detailed instructions. The service should be started now and you should be able to consume the functions provided by the extension.
   
  Since the ArcObjects extension is no longer in use, you can undeploy it now (see [Undeploy extensions](../deploy-extensions-java/#undeploy-extensions)).

### Also See
-   [Migration strategies](../migration-strategies-java/)
