...
@ArcGISExtension
@ServerObjectExtProperties(displayName = "Java Simple REST SOE With Capabilities",
		description = "Java Simple REST SOE With 2 Capabilities",
		properties = "" ,
		allSOAPCapabilities = {"BusServices", "TrainServices" },
		defaultSOAPCapabilities = { "BusServices" },
		servicetype = "MapService",
		supportsSharedInstances = false)

public class JavaSimpleRESTSOEWithCapabilities implements IServerObjectExtension, IRESTRequestHandler
	 {
	...
	public void shutdown()throws IOException,AutomationException{
	 ...
	 private JSONObject getLayerInfo() throws Exception {
		 ...
		 for (int i = 0; i < count; i++) {
			 ...
			 if (layerInfo.isFeatureLayer()) {
				 FeatureClass fc = new FeatureClass(this.mapServerDataAccess.getDataSource(ms.getDefaultMapName(), id));
	  ...
	  *
	  * @return JSON representation of NumberOfBusStations resource's
	  *         description.
	  */
	  ...
	  * description. This resource is not a collection.
	  *
	  * @return JSON representation of NumberOfTrainStations resource's
	  *         description.
	  */
	  ...
	  *
	  * @return JSON representation of specified resource.
	  */
