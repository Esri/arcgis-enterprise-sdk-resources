...
namespace NetUNTracingRESTSOE
{
  ...
  public class NetUNTracingRESTSOE : IServerObjectExtension, IObjectConstruct, IRESTRequestHandler
  {
    ...
    private const string DOMAIN_NETWORK = "Electric";

    // Medium Voltage Tier Name and Code
    private const string MV_TIER_NAME = "Electric Distribution"; 
    ...
    private const int MV_XFR_ASSETGROUP = 38; 
    private const int MV_XFR_TERMINAL_ID = 8; 

    // Service Point Asset Group and Asset Types
    private const int LV_SERVICE_ASSETGROUP = 22;
    public readonly int[] LV_SERVICE_ASSETTYPES = { 421, 422, 423, 424 }; 
    ...
    public void Init(IServerObjectHelper pSOH)
    {
      ...
      if (unLayerInfo != null)
      {
        ...
        unDataset = (IDataset)mapServerDataAccess.GetDataSource(mapService.DefaultMapName, unLayerInfo.ID);      
    ...
    */
    private byte[] GetTransformersOperHandler(NameValueCollection boundVariables, JsonObject operationInput,
    ...
    */
    private byte[] GetCustomersOperHandler(NameValueCollection boundVariables, JsonObject operationInput,
                                                  string outputFormat, string requestProperties, out string responseProperties)
    {
    ...
    */
    ...
    private IStringArray FindLVServicePoints(string xfrGlobalID)
    {
      ...
      // Get NetworkSource Id  
      int? networksourceId = soeUtil.GetNetworkSourceIdByFeatureClassUsageType(deUtilityNetwork, DOMAIN_NETWORK, esriUtilityNetworkFeatureClassUsageType.esriUNFCUTDevice);
      ...
      for (int i = 0; i < LV_SERVICE_ASSETTYPES.Length; i++)
      {
        ...
        outFilter.NetworkSourceID = (int)networksourceId;
